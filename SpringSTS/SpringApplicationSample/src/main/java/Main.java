...
//		ItemRepository itemRepository1 = RepositoryFactory.create();
		...
		try (GenericXmlApplicationContext context = new GenericXmlApplicationContext("applicationContext.xml")){
			/*
			 * // bean 태그의 id와 클래스 이름을 기재 ItemRepository itemRepository =
			 * context.getBean("itemRepository", ItemRepository.class); Item item =
			 * itemRepository.get(); System.out.println(item);
			 */
