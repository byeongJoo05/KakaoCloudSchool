...
class Iteration extends Component {
    ...
    handleRemove = (index) => {
        ...
        const {names} = this.state; // state 중에서 names 만 들고 올 수 있음
        // const names = this.state.names; // names 속성만 가져오는 것
    ...
    render() {
        const nameList = this.state.names.map((name, index) => 
            <li key={index} onDoubleClick={(e)=>{this.handleRemove(index)}}>{name}<button onClick={(e) => this.handleRemove(index)}>삭제</button></li>
        );
